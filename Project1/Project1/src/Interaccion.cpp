...
void Interaccion::Rebote(Humano& h, Caja c) {
	...
	float x_min = c.suelo.limite1.x;

	if (h.posicion.x > x_max) h.posicion.x = x_max;
	if (h.posicion.x < x_min) h.posicion.x = x_min;
}

bool Interaccion::Rebote(Esfera& e, Pared p) {
	Vector2D dir;  
	float dif = p.Distancia(e.posicion, &dir) - e.radio; //calcula la distancia entre la Pared y la esfera y le resta el radio  
...
bool Interaccion::Colision(Esfera e, Humano h) {
	Vector2D pos = h.GetPos();//pos base hombre
	pos.y += h.GetAltura()/2.0f;//pos centro hombre

	float dist = (e.posicion - pos).Modulo();
	if (dist < e.radio)
		return true;
...
bool Interaccion::Colision(Disparo d, Pared p) {
	Vector2D pos;
	float dif = p.Distancia(d.GetPos(), &pos) - d.GetRadio();
	if (dif <= 0) {
		return true;
	}
	else
